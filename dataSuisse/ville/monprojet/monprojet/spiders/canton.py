...
class CantonSpider(scrapy.Spider):
    name = 'canton'

    def start_requests(self):
        # Chemin absolu vers le fichier HTML
        file_path = r'C:\Users\MADA-Digital\dev\Scrapy\dataSuisse\ville\monprojet\monprojet\canton.html'
        ...
        for link in selector.css('div.dropdown-content a.dropdown-item'):
            yield {
                'nom_canton': link.css('::text').get().strip() if link.css('::text').get() else '',
